...
const RekamMedis = ({ data, setData }) => {
  ...
  const handleDelete = (rowId) => {
    ...
    const [parentId, riwayatIndex] = rowId.split("-").map(Number);

    setData((prevData) =>
      prevData
        .map((item) => {
          if (item.id === parentId) {
            const newRiwayat = [...item.riwayat];
            newRiwayat.splice(riwayatIndex - 1, 1);
            return { ...item, riwayat: newRiwayat };
          }
          return item;
        })
        .filter((item) => item.riwayat.length > 0)
    );
  };
